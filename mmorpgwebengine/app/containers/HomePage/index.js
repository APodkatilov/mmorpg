...
export function HomePage({
  ...
  const _websocketConnect = () => {    
    ws =new WebSocket(`ws://localhost:3031?token=${token}`);
    ws.onopen = () => {
    ...
    ws.onmessage = evt => {
      ...
      setMessages([...this.state.messages, evt.data]);
    }
  ...
  return (
    <article>
      ...
      <div>
      <div>
        <button onClick={() => _websocketConnect()}>Connect</button>
        <button onClick={() => _sendMessage()}>Send</button>
        <button onClick={() => _close()}>Close</button>
        <button onClick={() => _clear()}>Clear</button>
        <button onClick={() => _login()}>Singin</button>

        <div>
          <span>Message:</span> 
          <div>{messages.map((m, index) => <p key={index}>{m}</p>)}</div>
        </div>
        <div>
          <span>Token:</span>
